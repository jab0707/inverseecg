...
costs=nan(1,alpharate);
val = nan(1);
repeat = 1;
initalphamax = alphamax;
alphrepeat = 0;
while repeat
	alphas = linspace(alphamax/alpharate,alphamax,alpharate);
	for i=1:numel(alphas),costs(i)=phialpha(alphas(i));end
	[val,ind]=min(costs);
	alphamax = alphas(1);
	if ~isnan(val)
		if (numel(find(costs < phi(x))) >= 3)
			repeat = 0;
		elseif  val < phi(x)
			alphamax = (1+0.1)*max(alphas(costs< phi(x)));
		elseif val == phi(x)
			alphamax = initalphamax;
			alpharate = alpharate*10;
			if alphrepeat == 1
				repeat = 0;
			end
			alphrepeat = alphrepeat +1;
		end
	end
end
alphafinal=alphas(ind);
% plot(alphas,costs);hold on;plot(alphafinal,val,'rx');hold off;
% pause;
return
